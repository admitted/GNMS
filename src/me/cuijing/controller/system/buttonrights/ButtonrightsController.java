package me.cuijing.controller.system.buttonrights;
...
public class ButtonrightsController extends BaseController {
	...
	public ModelAndView list() throws Exception {
		...
		if (pd.getString("ROLE_ID") == null || "".equals(pd.getString("ROLE_ID").trim())) {
			pd.put("ROLE_ID", "1");        //默认列出第一组角色(初始设计 系统组组 会员用户组不能删除)
		}
		PageData fpd = new PageData();     //
		fpd.put("ROLE_ID", "0");
		List<Role> roleList = roleService.listAllRolesByPId(fpd);           //列出全部角色组(角色组:系统组组 会员用户组)
