...
public class Jurisdiction {
	...
	public static boolean hasJurisdiction(String menuUrl){
		...
		List<Menu> allMenuList = (List<Menu>)session.getAttribute(USERNAME + Const.SESSION_allmenuList); //获取所有菜单列表
		return readMenu(allMenuList,menuUrl,session,USERNAME);
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static boolean readMenu(List<Menu> allMenuList,String menuUrl,Session session,String USERNAME){
		for(int i = 0; i < allMenuList.size() ; i++){
			...
			if( allMenuList.get(i).getMENU_URL().split(".do")[0].equals(menuUrl.split(".do")[0]) ){
				...
				if(!allMenuList.get(i).isHasMenu()){		// 判断当前用户 有无此菜单权限
					...
					map.remove("edit");
					map.remove("cha");
					String MENU_ID =  allMenuList.get(i).getMENU_ID();
				...
				// 继续排查其子菜单
				if(!readMenu(allMenuList.get(i).getSubMenu(),menuUrl,session,USERNAME)){
					return false;
				}
			}
		}
	...
	public static boolean buttonJurisdiction(String menuUrl, String type){
		...
		String USERNAME = getUsername();	// 获取当前登录者loginname
		Session session = getSession();
		List<Menu> menuList = (List<Menu>)session.getAttribute(USERNAME + Const.SESSION_allmenuList); // 获取全部菜单列表
	...
	public static boolean readMenuButton(List<Menu> menuList,String menuUrl,Session session,String USERNAME, String type){
		for(int i=0;i<menuList.size();i++){
			...
			if(menuList.get(i).getMENU_URL().split(".do")[0].equals(menuUrl.split(".do")[0])){
				if(!menuList.get(i).isHasMenu()){ // isHasMenu 为 FALSE : 判断有无此菜单权限 菜单是否开启??
					return false;
				}else{	// 按钮判断
					// 按钮权限(增删改查)
					Map<String, String> map = (Map<String, String>)session.getAttribute(USERNAME + Const.SESSION_QX);
	...
	/**
	 * shiro管理的会话session [不单单可以应用到Web框架]
	 * @return
	 */
